...
# ///
"""
blog CLI - Setup tool for blog-Kit projects

Usage:
    uvx blog-cli.py init <project-name>
    uvx blog-cli.py init .
    uvx blog-cli.py init --here

Or install globally:
    uv tool install --from blog-cli.py blog-cli
...
TAGLINE = "Blog-Tech-Kit - Blog Content Strategy Toolkit"
class StepTracker:
...
app = typer.Typer(
    name="blog",
    help="Setup tool for blog-Kit blog discovery projects",
...
def callback(ctx: typer.Context):
    ...
    if ctx.invoked_subcommand is None and "--help" not in sys.argv and "-h" not in sys.argv:
        show_banner()
        console.print(Align.center("[dim]Run 'blog --help' for usage information[/dim]"))
        console.print()
...
def check_tool(tool: str, tracker: StepTracker = None) -> bool:
    ...
    # See: https://github.com/agentii-ai/blog-kit/issues/123
...
def download_template_from_github(ai_assistant: str, download_dir: Path, *, script_type: str = "sh", verbose: bool = True, show_progress: bool = True, client: httpx.Client = None, debug: bool = False, github_token: str = None) -> Tuple[Path, dict]:
    ...
    try:
        ...
        # If blog-kit has no releases yet (404), fall back to spec-kit templates
        if status == 404:
            if verbose:
                console.print("[yellow]blog-Kit releases not found, falling back to Spec-Kit templates...[/yellow]")
            repo_owner = "github"
            repo_name = "spec-kit"
            api_url = f"https://api.github.com/repos/{repo_owner}/{repo_name}/releases/latest"
            response = client.get(
...
def ensure_executable_scripts(project_path: Path, tracker: StepTracker | None = None) -> None:
    """Ensure POSIX .sh scripts under .blog/scripts (recursively) have execute bits (no-op on Windows)."""
    if os.name == "nt":
        return  # Windows: skip silently
    scripts_root = project_path / ".blog" / "scripts"
...
def init(
    ...
    """
    Initialize a new blog project from the latest template.
    ...
    tracker = StepTracker("Initialize blog Project")
    ...
    steps_lines.append("   2.1 [cyan]/blogkit.constitution[/] - Establish project principles")
    steps_lines.append("   2.2 [cyan]/blogkit.specify[/] - Create baseline specification")
    steps_lines.append("   2.3 [cyan]/blogkit.plan[/] - Create implementation plan")
    steps_lines.append("   2.4 [cyan]/blogkit.tasks[/] - Generate actionable tasks")
    steps_lines.append("   2.5 [cyan]/blogkit.implement[/] - Execute implementation")
    ...
    enhancement_lines = [
        ...
        "",
        f"○ [cyan]/blogkit.clarify[/] [bright_black](optional)[/bright_black] - Ask structured questions to de-risk ambiguous areas before planning (run before [cyan]/blogkit.plan[/] if used)",
        f"○ [cyan]/blogkit.analyze[/] [bright_black](optional)[/bright_black] - Cross-artifact consistency & alignment report (after [cyan]/blogkit.tasks[/], before [cyan]/blogkit.implement[/])",
        f"○ [cyan]/blogkit.checklist[/] [bright_black](optional)[/bright_black] - Generate quality checklists to validate requirements completeness, clarity, and consistency (after [cyan]/blogkit.plan[/])"
    ]
...
def check():
    ...
    console.print("\n[bold green]blog CLI is ready to use![/bold green]")
...
def version():
    ...
    cli_version = "unknown"
    try:
        cli_version = importlib.metadata.version("blog-cli")
    except Exception:
    ...
    panel = Panel(
        info_table,
        title="[bold cyan]blog CLI Information[/bold cyan]",
        border_style="cyan",
        padding=(1, 2)
    )
