...
- ✅ **Script files** (`.blog/scripts/`)
- ✅ **Template files** (`.blog/templates/`)
- ✅ **Shared memory files** (`.blog/memory/`) - **⚠️ See warnings below**
...
**Known issue:** `specify init --here --force` currently overwrites `.blog/memory/constitution.md` with the default template, erasing any customizations you made.

**Workaround:**

```bash
# 1. Back up your constitution before upgrading
cp .blog/memory/constitution.md .blog/memory/constitution-backup.md

# 2. Run the upgrade
specify init --here --force --ai copilot

# 3. Restore your customized constitution
mv .blog/memory/constitution-backup.md .blog/memory/constitution.md
```
...
git restore .blog/memory/constitution.md
```

### 2. Custom template modifications

If you customized any templates in `.blog/templates/`, the upgrade will overwrite them. Back them up first:

```bash
# Back up custom templates
cp -r .blog/templates .blog/templates-backup
...
rm blogkit.specify-old.md
rm blogkit.plan-v1.md
```
...
cp .blog/memory/constitution.md /tmp/constitution-backup.md
cp -r .blog/templates /tmp/templates-backup
...
mv /tmp/constitution-backup.md .blog/memory/constitution.md
...
rm blogkit.old-command-name.md
...
cp /tmp/constitution-backup.md .blog/memory/constitution.md
```
...
3. **Some files may be overwritten** - If you already have Spec Kit files (`.claude/`, `.blog/`, etc.), they'll be replaced with the new versions

**What gets overwritten:**

Only Spec Kit infrastructure files:
- Agent command files (`.claude/commands/`, `.github/prompts/`, etc.)
- Scripts in `.blog/scripts/`
- Templates in `.blog/templates/`
- Memory files in `.blog/memory/` (including constitution)
...
**Prevention tip:** Before upgrading, commit or back up your `.blog/memory/constitution.md` if you customized it.
...
Once you've run `specify init`, the slash commands (like `/blogkit.blog`, `/blogkit.plan`, etc.) are **permanently installed** in your project's agent folder (`.claude/`, `.github/prompts/`, etc.). Your AI assistant reads these command files directly—no need to run `specify` again.
...
- **Test new slash commands:** Run `/blogkit.constitution` or another command to verify everything works
